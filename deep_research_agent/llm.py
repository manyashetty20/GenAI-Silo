...
def _get_hf_pipeline() -> Any:
    ...
    if not cfg.hf_model_name_or_path:
        raise RuntimeError(
            "LLM backend is set to 'hf_local' but hf_model_name_or_path is not configured in config.LLMConfig."
        )
    ...
    # Optionally load LoRA adapter if provided
    if cfg.hf_lora_path:
        try:
            from peft import PeftModel

            model = PeftModel.from_pretrained(model, cfg.hf_lora_path)
            # Merge LoRA weights into base model for inference stability
            model = model.merge_and_unload()
        except ImportError as exc:
            raise RuntimeError(
                "hf_lora_path is set but 'peft' is not installed. "
                "Add 'peft' to requirements and reinstall."
            ) from exc
...
def invoke(
    ...
    # 1. OpenAI / OpenAI-compatible HTTP backend
    if cfg.backend == "openai":
        llm = _get_openai_client(temperature=temperature, max_tokens=max_tokens, model=None)
    ...
    # 2. Local HF (optionally LoRA-adapted)
    if cfg.backend == "hf_local":
        pipe = _get_hf_pipeline()
        full_prompt = f"{system}\n\n{prompt}" if system else prompt
        gen_kwargs: dict[str, Any] = {
            "max_new_tokens": max_tokens or cfg.max_tokens,
            "temperature": max(0.01, temperature if temperature is not None else cfg.temperature),  # Prevent zero temp
            "top_p": 0.95,
            "top_k": 50,
            "do_sample": True,
        }
